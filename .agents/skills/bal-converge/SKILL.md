...
게임-종속 정보는 `.Codex/bal-converge.json` 한 파일에만 (metric 별칭, 디폴트 KPI). 다른 게임에 그대로 옮겨갈 수 있다.
...
[6] 종료 보고 (사이클별 metric 표 + REASON + 누적 MODIFIED 파일 + git diff --stat)
        ↓
[7] HTML 리포트 생성 (playbalance/reports/converge_<ts>_<reason>.html)
      - 사이클·knob 데이터 + Codex가 작성한 narrative(대화/분석/결론) → JSON
      - make_report.py 호출 → 단일 자립 HTML (외부 의존 0개)
      - pass 든 abort 든 항상 생성 (autopsy 가치)
```
...
# (5) 두 의존 skill 존재
[ -f .Codex/skills/bal-run/SKILL.md ]   || die "/bal-run skill이 없습니다."
[ -f .Codex/skills/bal-apply/SKILL.md ] || die "/bal-apply skill이 없습니다."

# (6) 두 의존 config 존재
[ -f .Codex/bal-run.json ]   || die "/bal-run 설정 없음. 먼저 /bal-run 한 번으로 config 생성."
[ -f .Codex/bal-apply.json ] || die "/bal-apply 설정 없음. 먼저 /bal-apply dry-run으로 cold start 통과 또는 수동 작성."
...
if [ "$HAS_CMUX" = "1" ]; then
  ...
  # 2. dashboard pane 2개 생성 (결정론적 레이아웃: Codex(좌) | GAME(우상) / CONVERGE(우하)):
  ...
  #       둘 다 caller(Codex) pane 기준으로 분할되어 우상/우하 레이아웃이 안 만들어진다.
  ...
  # (a) Codex 오른쪽으로 새 browser pane → GAME (우상단)
...
- 레이아웃 규약: `Codex(좌) | GAME(우상) / CONVERGE(우하)`. 시선 흐름은 위(현재 iter 실시간) → 아래(누적 KPI 거동). 이 순서는 `new-pane` 두 번 사이에 `focus-pane` 으로 새 pane 에 명시적 focus 를 옮겨야 결정론적으로 만들어진다 — `new-pane` 은 자동 focus 이동을 하지 않는다.
...
[ "$PLAN_COUNT" -eq 0 ] && die "[bal-converge] 진단 결과 적용할 knob이 0개입니다 (모든 finding이 trigger 매칭 0이거나 자산 단정 실패). KPI는 이미 충족? KPI를 다시 확인하거나 /bal-apply config(.Codex/bal-apply.json)에 knob을 추가하세요."
```
...
`.Codex/bal-converge.json` 의 `display_labels` 필드(있으면)에서 메트릭/knob의 한국어 라벨과 자세한 설명을 가져온다. 없으면 영문 alias 그대로 폴백 — 게임-비종속 유지.

```bash
LABELS_JSON=$(python3 -c "
import json
c = json.load(open('.Codex/bal-converge.json'))
...
if [ -n "$CONVERGE_SURF" ] && [ -n "$CONVERGE_SID" ]; then
  ...
  # chart_keys chips 자동 추가 (.Codex/bal-run.json 에서 읽어 재사용)
  # 시계열 차트가 비어있지 않도록 게임-메트릭 시계열 키들을 chip으로 채운다.
  # bal-run.json 이 없거나 chart_keys 가 비어있으면 silent skip.
  if [ -f .Codex/bal-run.json ]; then
    CHART_KEYS=$(jq -r '.chart_keys[]?' .Codex/bal-run.json 2>/dev/null)
    CHIP_COUNT=0
...
update_game_pane() {  # SID
  ...
  if [ -f .Codex/bal-run.json ]; then
    for K in $(jq -r '.chart_keys[]?' .Codex/bal-run.json 2>/dev/null); do
      [ -z "$K" ] && continue
...
case "$REASON" in
  ...
  diverged) echo "  ! KPI distance가 사이클마다 증가했습니다. knob 매칭이 잘못됐을 가능성. .Codex/bal-apply.json 검토 또는 KPI를 더 도달 가능한 값으로." ;;
  no_op) echo "  - 적용할 knob이 0개로 떨어졌습니다 (모든 finding이 trigger 매칭 0). 새 knob 추가 검토." ;;
  apply_abort_2) echo "  ! /bal-apply config 없음 (cold start). .Codex/bal-apply.json 작성." ;;
...
종료 보고 print 직후, 마지막 단계로 **HTML 리포트를 생성한다**. pass / abort / no_op / max_iter / max_wall / diverged / apply_abort_* / bal_run_failed 모두 — 어떤 REASON 으로든 항상 생성. 실패 케이스의 autopsy 가 다음 세션의 핵심 정보.

저장 위치는 `playbalance/reports/converge_<YYYYMMDD>-<HHMMSS>_<reason>.html`. 폴더 컨벤션은 `playbalance/README.md` 참조.

### JSON payload 작성 (Codex 가 종료 시점에 채움)

사이클 루프 동안 누적해둔 데이터 + Codex 의 narrative 를 한 JSON 으로 합쳐 임시파일에 쓴 뒤 make_report.py 에 넘긴다.
...
```json
{
  ...
  "version": "<VERSION>",
  "branch": "<git branch>",
  "timestamp_iso": "2026-05-22T14:45:30",
  "kpi_expression": "<원본 KPI 문자열>",
  "max_iter": 5, "max_wall_sec": 1800, "wall_sec": <actual>,
  "reason": "pass | max_iter | max_wall | diverged | no_op | apply_abort_2 | apply_abort_3 | bal_run_failed",
  "converge_session_id": "<converge_run_* session id, 없으면 빈 문자열>",
  "playtrace_url": "http://localhost:8000",
  "metric_aliases": {"Dur": "episode.duration_sec", ...},
  "display_labels": <.Codex/bal-converge.json 의 display_labels 그대로>,
  "modified_assets": ["Assets/.../X.asset", ...],
  "cycles": [
    {
      "iter": 0,
      "session_id": "<bal-run session>",
      "is_baseline": true,
      "metrics": {"Dur": 64.5, "Lvl": 3.0},
      "distance": 2.32,
      "pass": false,
      "term_results": [   // 마지막 사이클 (또는 baseline) 에만 채우면 충분
        {"name":"Dur","kind":"range","target_min":180,"target_max":240,"current":64.5,"pass":false,"distance":1.92},
        {"name":"Lvl","kind":"cmp","target_op":">=","target_val":5,"current":3.0,"pass":false,"distance":0.40}
      ],
      "applied_knobs": []   // baseline 은 빈 배열
    },
    {
      "iter": 1, "session_id": "...", "metrics": {...}, "distance": 0.91, "pass": false,
      "applied_knobs": [
        {"knob":"enemy_damage","asset":"Assets/.../Gear.asset",
         "field":"ValueDefinitions[ValueName=Attack].DefaultValue",
         "from":5, "to":4, "why":"killer_concentration finding ..."}
      ]
    }
  ],
  "narrative": {
    "chat_summary": {
      "kpi_rationale": "...왜 이 KPI를 골랐는지 (대화에서 추출)...",
      "prior_hypotheses": ["...", "..."],
      "user_concerns": ["...", "..."],
      "invocation_summary": "(위 3개가 다 비어있을 때만) '사용자가 KPI X 로 호출. 추가 논의 없음.' 같은 fallback 한 줄"
    },
    "data_analysis_extra": {
      "baseline_observation": "1 문단 — baseline 진단 결과를 narrative로",
      "key_observation": "1 문단 — impact ranking 후 핵심 관찰",
      "signals": ["...", "..."]
    },
    "conclusion": {
      "validated": ["...", "..."],
...
스키마 메모:
- `cycles` 가 빈 배열이면 (apply_abort_2 같은 케이스) 리포트는 헤더 + 요약 + autopsy narrative만 렌더. KPI/추이/표/이력 섹션은 자동 생략.
- 사이클 1개(baseline only) 면 추이 차트만 생략됨 (포인트 1개로 라인 그릴 수 없음).
- `term_results` 는 baseline 과 마지막 사이클에만 채우면 KPI gauge 섹션 표시에 충분. 중간 사이클은 생략 가능.
- `narrative.chat_summary` 는 **항상** 한 박스를 렌더. 대화 컨텍스트가 빈약하면 `invocation_summary` 한 줄만 채워도 됨.
- `narrative.data_analysis_extra` 는 모든 필드 옵션. 사이클 0개 케이스에서는 통째로 생략 가능.

### narrative 작성 가이드 (Codex 가 종료 시점에)

**`chat_summary`** — 대화 로그 요약. 이번 세션에서 진짜 나온 발화만. hallucination 금지:
- KPI 가 디폴트 (`default_kpi`) 면 `kpi_rationale` 비우거나 "디폴트 KPI 그대로 사용" 한 줄.
- 사용자가 KPI 를 명시 또는 수정했으면 그 이유를 대화에서 1~2문장 요약.
- 사전 가설/우려는 대화에서 실제 언급된 것만. 안 나왔으면 빈 배열.
- 자율 호출 (대화 컨텍스트 없음) 시: `invocation_summary` 만 한 줄 "사용자가 KPI X 로 호출. 추가 논의 없음." (사용자 결정 #2 = C).

**`data_analysis_extra`** — 데이터 해석:
- `baseline_observation`: baseline 사이클의 진단 결과 1~2문장. "사망 원인 분포", "structural cutoff 신호 (σ/μ)", "메트릭과 KPI 거리" 같은 사실 관찰.
- `key_observation`: 자동 계산된 impact ranking 직후 보일 1~2문장 해석. 가장 효과 큰 knob, 가설 검증 여부.
- `signals`: distance 단조성, knob 변화율이 가이드라인 안에 있는지, overfit 의심 등 진행 신호 bullet 2~4개.

**`conclusion`** — 결론과 후속:
- `validated`: 이번 사이클이 데이터로 입증한/약화시킨 가설.
- `next_hypotheses`: 다음 세션이 할 일. "N 늘려 재검증", "다른 t의 spawn pattern 확인", "추가 KPI 후보" 등.

### 호출 코드

```bash
TS=$(date +%Y%m%d-%H%M%S)
OUT="playbalance/reports/converge_${TS}_${REASON}.html"
mkdir -p playbalance/reports

# Codex 가 위 스키마대로 JSON 작성해서 임시파일에 저장
JSON_FILE="/tmp/converge_report_${TS}.json"
# (Write 도구로 JSON_FILE 작성 — 본문은 사이클 누적 + narrative)

python3 .Codex/skills/bal-converge/make_report.py --in "$JSON_FILE" --out "$OUT"
echo ""
echo "📊 리포트: $OUT"
[ "$HAS_CMUX" = "1" ] && cmux toast "bal-converge 리포트 생성: $OUT"
rm -f "$JSON_FILE"
```

리포트 생성 실패 (스키마 누락, 파일 권한 등) 시 사이클 결과 자체는 이미 끝났으므로 warn 만 띄우고 비-치명적으로 처리. 사용자가 수동으로 JSON 다시 만들어 재호출 가능.

---

## 게임-비종속

KPI 파서, distance 함수, 사이클 로직, divergence 감지, **리포트 생성기 (`make_report.py`)** 모두 게임 정보 0. 게임-종속은:
- `.Codex/bal-converge.json` 의 `metric_aliases` (alias → PlayTrace key)
- `default_kpi` (이 게임 특유의 목표)
- `display_labels` (메트릭/knob 의 한국어 라벨 — 리포트의 KPI 게이지·impact ranking 에서 사용)

새 게임 이식 시 위 필드들만 갈아끼우면 된다 (해당 게임에 `/bal-run`, `/bal-apply` skill 셋업이 먼저 되어 있어야 함). `make_report.py` 와 `playbalance/` 폴더 구조는 그대로 복사.
...
- **`--auto` 무조건 신뢰 금지**: `--auto`는 사용자 보호를 위해 보수적으로 skip 많이 함. 자주 no_op로 종료된다면 .Codex/bal-apply.json knob에 `asset_match` 추가가 필요할 가능성.
- **PlayTrace `play_end_key` 컨벤션 가정**: 위 fetch_metrics 는 `episode.cause` 하드코딩. 다른 게임에서는 `.Codex/bal-run.json` 의 `play_end_key` 를 읽어야 함. 향후 metric_aliases 옆에 `play_end_key` 도 bal-converge config에 두는 게 깔끔.
- **bal-run discovery 호출**: bal-converge 사이클 안에서 bal-run을 처음 호출했는데 config(.Codex/bal-run.json) 가 없으면 bal-run이 discovery flow로 빠져 AskUserQuestion을 띄움 — bal-converge의 무인 사이클을 멈춤. 사전 점검 (6)에서 막음.
...
- **리포트 narrative hallucination**: `narrative.chat_summary` 는 **이번 세션의 실제 대화** 에서만 추출. 대화 안 한 가설/우려를 사용자 입에 넣지 말 것. 대화 컨텍스트가 빈약하면 `invocation_summary` 한 줄 fallback (사용자 결정 #2 = C). 6개월 뒤 리포트를 다시 봤을 때 "내가 이런 말 한 적 있나?" 가 되면 honesty guard 깨진 거.
- **리포트 누락 시 사이클 결과는 살아있음**: HTML 생성이 실패해도 (스키마 누락, 권한 등) `git status` 에 자산 변경은 그대로 있고 PlayTrace 의 converge_run / 사이클별 session 들도 그대로. 사용자가 수동으로 JSON 재작성 → make_report.py 재호출로 리포트만 다시 만들 수 있음. 사이클 자체를 다시 돌릴 필요 없음.
...
- `.Codex/skills/bal-run/SKILL.md` — N회 플레이 + 데이터 수집. session ID는 `session=<id>` 형태로 stdout emit.
- `.Codex/skills/bal-apply/SKILL.md` — `--auto` 분기 표, `MODIFIED:`/`PLAN:` emit 규칙, `probe_field_value`.
- `.Codex/skills/bal-converge/make_report.py` — 게임-비종속 HTML 리포트 생성기. stdlib only. 자세한 schema 는 위 "리포트 생성" 섹션.
- `.Codex/bal-converge.json` — KPI/한도/metric_aliases/display_labels (이 skill의 캐시).
- `playbalance/README.md` — 리포트 저장 폴더 컨벤션 (파일명 규칙, 형식, 보는 법).
