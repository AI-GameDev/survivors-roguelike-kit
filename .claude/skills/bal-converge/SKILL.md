...
[1] 인자 + config 로드 (KPI, 한도)
        ↓
[1.5] cmux dashboard pane: 기존 browser surface 모두 정리 → 새 pane 1개 생성
        ↓
...
[4] Plan Preview 출력 + AskUserQuestion (Proceed / Modify KPI / Abort)
        ↓ (Proceed)
[4.5] converge_run session 생성 → dashboard pane selectors 설정 + chart_keys chips 자동 추가
        ↓
...
bal-converge 는 진행 상황을 실시간으로 보기 위해 **사이클 시작 시점에 dashboard 브라우저 pane을 명시적으로 1개 띄운다**. 이전 실행 / 수동 실험 / 다른 skill 호출에서 남은 dashboard pane이 워크스페이스에 누적되는 걸 피하기 위해 **시작 시 현재 workspace 의 모든 browser surface를 닫고 새로 1개** 만든다.
...
if [ "$HAS_CMUX" = "1" ]; then
  ...
  # 2. dashboard 1개 새로 생성 (오른쪽 split, browser pane)
  NEW_OUT=$(cmux new-pane --type browser --direction right --url "http://localhost:8000/dashboard" 2>&1)
  DASH_SURF=$(echo "$NEW_OUT" | grep -oE 'surface:[0-9]+' | head -1)

  if [ -z "$DASH_SURF" ]; then
    echo "[bal-converge] WARNING: dashboard pane 생성 실패. 사용자가 수동으로 열어주세요. (출력: $NEW_OUT)"
  else
    echo "[bal-converge] dashboard pane: $DASH_SURF"
    # converge_run session 생성 후 (아래 섹션) 이 surface 에 selectors 설정
  fi
fi
```
...
- `/bal-run` 은 사이클 안에서 자체적으로 `cmux browser open-split` 으로 추가 dashboard pane을 만든다. 이 skill의 cleanup은 사이클 시작 시점에 한 번만 실행되므로, 사이클이 끝나면 dashboard 가 2~3개로 누적될 수 있다. 사용자가 수동 정리하거나, 다음 `/bal-converge` 실행 시 자동 정리됨 (이 섹션의 step 1).
...
# dashboard pane (위 "Dashboard 패널 셋업" 섹션의 $DASH_SURF) 이 살아있으면
# 즉시 converge_run session 으로 selectors 채우기 + chart_keys chip 자동 추가
if [ -n "$DASH_SURF" ] && [ -n "$CONVERGE_SID" ]; then
  sleep 2  # 페이지 초기 로드 여유
  cmux browser select --surface "$DASH_SURF" "#sel-project" "$PROJECT" >/dev/null 2>&1 || true
  sleep 1
  cmux browser select --surface "$DASH_SURF" "#sel-version" "$VERSION" >/dev/null 2>&1 || true
  sleep 1
  cmux browser select --surface "$DASH_SURF" "#sel-session" "$CONVERGE_SID" >/dev/null 2>&1 || true
  echo "[bal-converge] dashboard → project=$PROJECT version=$VERSION session=$CONVERGE_SID"
  ...
  if [ -f .claude/bal-run.json ]; then
    ...
    for K in $CHART_KEYS; do
      [ -z "$K" ] && continue
      cmux browser fill --surface "$DASH_SURF" "#sel-key" "$K" >/dev/null 2>&1 || continue
      cmux browser press --surface "$DASH_SURF" "Enter" >/dev/null 2>&1 || continue
      CHIP_COUNT=$((CHIP_COUNT+1))
      sleep 0.3
    done
    [ "$CHIP_COUNT" -gt 0 ] && echo "[bal-converge] dashboard chart chips: ${CHIP_COUNT}개 추가"
  fi
fi
```
...
- **dashboard pane 누적**: 이 skill 의 "Dashboard 패널 셋업" 은 사이클 **시작 시점에 한 번만** 정리한다. 사이클 안의 `/bal-run` 호출은 자체적으로 추가 dashboard pane 을 만든다. 사이클 1회당 1~3개 잔여 가능. 다음 `/bal-converge` 실행이 다 정리하지만, 그 사이에 사용자가 직접 정리하고 싶으면 `cmux list-pane-surfaces | awk '/\[browser\]/ {print $2}' | xargs -I{} cmux close-surface --surface {}`.
