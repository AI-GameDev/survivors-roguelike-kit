...
[7] HTML 리포트 생성 (playbalance/reports/converge_<ts>_<reason>.html)
      - 사이클·knob 데이터 + Claude가 작성한 narrative(대화/분석/결론) → JSON
...
### JSON payload 작성 (Claude 가 종료 시점에 채움)

사이클 루프 동안 누적해둔 데이터 + Claude 의 narrative 를 한 JSON 으로 합쳐 임시파일에 쓴 뒤 make_report.py 에 넘긴다.
...
```json
{
  ...
  "display_labels": <.claude/bal-converge.json 의 display_labels 그대로>,
...
### narrative 작성 가이드 (Claude 가 종료 시점에)
...
# Claude 가 위 스키마대로 JSON 작성해서 임시파일에 저장
JSON_FILE="/tmp/converge_report_${TS}.json"
# (Write 도구로 JSON_FILE 작성 — 본문은 사이클 누적 + narrative)

python3 .claude/skills/bal-converge/make_report.py --in "$JSON_FILE" --out "$OUT"
echo ""
echo "📊 리포트: $OUT"
...
- `.claude/skills/bal-converge/make_report.py` — 게임-비종속 HTML 리포트 생성기. stdlib only. 자세한 schema 는 위 "리포트 생성" 섹션.
