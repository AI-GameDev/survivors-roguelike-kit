...
- **레거시 유지** (기본): 기존 위치는 그대로, 신규 학습부터 `ml-training/` 사용. CLAUDE.md에 두 위치 모두 명시.
...
CLAUDE.md가 있으면 다음 섹션을 추가 (없으면 생성):
...
| `CLAUDE.md` ML-Agents 섹션 | `grep "## ML-Agents 강화학습 환경"` 검색 → 있으면 갱신, 없으면 추가 |
