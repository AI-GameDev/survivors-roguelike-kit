---
name: bal-run
description: PlayTrace + cmux를 쓰는 게임의 inference 모델을 N회 자동 플레이로 검증하는 밸런스 테스트. 한 PlayTrace session 안에 play_no 1~N을 누적하고 각 한판에 timeout cap을 둔다. 프로젝트별 config(<project>/.claude/bal-run.json)에 키 매핑을 캐시해 두 번째 실행부터는 인터뷰 없이 조용히 동작한다. 사용자가 "/bal-run", "bal run", "밸런스 테스트", "플레이테스트 N번", "inference N번 돌려", "한판 X분 캡" 같은 표현을 쓰면 사용. 선행 조건: /bal-init 완료 (inference wiring + PlayTrace 통합).
...
json.dump(config, open(".claude/bal-run.json", "w"), indent=2, ensure_ascii=False)
```

`.claude/` 디렉토리가 없으면 `mkdir -p` 먼저. 저장 후 사용자에게 "config가 `.claude/bal-run.json` 에 저장됐습니다. 다음부터는 이 파일을 읽고 인터뷰 건너뜁니다." 안내.
...
CFG = json.load(open(".claude/bal-run.json"))
...
- **Config out of date**: 게임이 새 키를 추가하거나 기존 키 이름을 바꾸면 chart_keys/summary_columns가 빈 컬럼/누락 값이 됨. 사용자가 데이터에 의문이 들면 `.claude/bal-run.json` 갱신 안내.
