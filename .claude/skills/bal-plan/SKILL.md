...
[3] Baseline 후보 스캔 — PlayTrace /api/sessions 최근 ~7일
       · session_id, n_plays, mean Dur, mean Lvl, end-cause 분포 한 줄씩
       · 재사용 가능한 후보 vs 새로 측정 가능성 둘 다 살림
        ↓
[4] Trap surfacing — memory + .claude/bal-apply.json knob 목록 교집합
...
[6] Plan 파일 작성 + 추천 명령 출력
       ...
       · 마지막 줄에 copy-paste 가능한 `/bal-converge kpi=... N=... max_iter=...`
```
...
추출 직후 한 줄 보고:
```
[bal-plan] mode=full, kpi="<from arg or default>", baseline=auto
```
...
[ -f .claude/bal-converge.json ] || die "/bal-converge config 없음 (.claude/bal-converge.json). /bal-converge 를 한 번 실행해 seed 를 만든 뒤 다시 시도."
[ -f .claude/bal-apply.json ]    || die "/bal-apply config 없음 (.claude/bal-apply.json). /bal-apply 의 cold start 를 먼저 통과시키세요."
[ -f .claude/bal-run.json ]      || die "/bal-run config 없음. /bal-run 을 한 번 돌려 config 를 만든 뒤 다시 시도."
...
리포트 HTML 안에는 make_report.py 가 inline 으로 박아둔 JSON payload 가 있다 (한 번 read 후 `<script id="report-data" type="application/json">` 같은 패턴 또는 본문의 KPI 헤더 라인). 정확한 위치는 `.claude/skills/bal-converge/make_report.py` 참고. 파싱이 까다로우면 fallback 으로 사람이 읽을 수 있는 헤더 텍스트만 인용해도 충분.
...
SESSIONS=$(curl -s -m 5 "http://localhost:8000/api/sessions?project_name=$(jq -r .project_name .claude/bal-run.json)&size=50" \
  | python3 -c "
...
각 후보 session 에 대해 한 줄 요약 (logs/search 로 mean Dur / Lvl / end-cause 분포):

```
Baseline 후보 (최근 7일, completed plays >= 3 인 것만):
  20260522_005  N=6  Dur=63.5±5.1s  Lvl=3.2  end=Gear(83%) Ghost(17%)   ← bal-converge 16:19 의 baseline 재사용본
  20260521_009  N=5  Dur=72.1±8.3s  Lvl=3.4  end=Gear(60%) Ghost(40%)
  20260520_018  N=10 Dur=58.0±2.1s  Lvl=2.9  end=Gear(100%)              ← σ/μ=3.6%, structural cutoff 의심
```
...
memory index (`~/.claude/projects/.../memory/MEMORY.md`) 의 feedback/project 항목 중 **이 프로젝트의 knob 과 직접 관계 있는 것** 을 골라 사용자에게 상기.

`.claude/bal-apply.json` 의 knob id 목록을 먼저 추출:

```bash
KNOB_IDS=$(jq -r '.knobs[].id' .claude/bal-apply.json)
```
...
- 전체 허용 (`.claude/bal-apply.json` 의 모든 knob)
...
knob whitelist 선택을 .claude/bal-apply.json 의 실제 knob id 목록과 다른 이름으로 추측하지 말 것. 사용자가 "Gear 만" 같이 자연어로 답하면 knob id 와 매칭되는 후보를 한 번 더 confirm.
...
### 예산
N=<n>, max_iter=<m>. 예상 wall-time ≈ <m × (n × per-play timeout + apply 30s)>.
...
- baseline 후보는 PlayTrace API + `.claude/bal-run.json:project_name` 만 사용
- knob 목록은 `.claude/bal-apply.json` 에서 동적으로 읽음
- KPI 디폴트 / 라벨은 `.claude/bal-converge.json` 에서 읽음
...
- **knob id hallucination 금지**: whitelist 작성 시 반드시 `.claude/bal-apply.json` 의 실제 id 만 사용. "Gear 약화" 같은 자연어를 받으면 어떤 knob id 와 매칭되는지 한 번 더 사용자에게 confirm.
...
- `.claude/skills/bal-converge/SKILL.md` — 인자 문법, KPI 표현식 syntax, 산출물 위치
- `.claude/skills/bal-apply/SKILL.md` — knob 구조, 자동/수동 분기
- `.claude/skills/bal-run/SKILL.md` — `chart_keys`, `play_end_key` 컨벤션
- `.claude/bal-converge.json` — default_kpi, display_labels
- `.claude/bal-apply.json` — knob 목록 (whitelist 선택의 단일 진실 소스)
- `.claude/bal-run.json` — `project_name` (PlayTrace 쿼리), `play_end_key`
- `playbalance/README.md` — 폴더 컨벤션 (`plans/` 도 같은 규약)
- `PlayTrace/docs/MANUAL.md` — `/api/sessions`, `/api/logs/search` payload·필드의 단일 진실 소스
- `~/.claude/projects/.../memory/MEMORY.md` — 누적 trap 인덱스
