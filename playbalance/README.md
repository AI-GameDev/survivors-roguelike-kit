...
playbalance/
├── README.md                       # 이 파일
├── plans/                          # /bal-plan 산출물 (bal-converge 직전 방향성 정리)
│   └── plan_<YYYYMMDD>-<HHMMSS>.md
└── reports/
...
| Skill | 쓰는 산출물 |
|---|---|
| `/bal-plan` | `plans/plan_*.md` — converge 직전 방향성·KPI·knob whitelist 정리 (구현됨) |
| `/bal-converge` | `reports/converge_*.html` — KPI 자동 수렴 사이클 전체 기록 (구현됨) |
| `/bal-run` | `reports/run_*.html` (예정 — 현재 미구현) |
| `/bal-apply` | `reports/apply_*.html` (예정 — 현재 미구현) |
